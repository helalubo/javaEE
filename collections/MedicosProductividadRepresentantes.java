...
import plataforma.domain.model.MedicoDomicilio;
...
public class MedicosProductividadRepresentantes extends ResultSetObject<Medico> {
	...
	public Medico getObject()  {
		Medico m = new Medico();
		MedicoDomicilio domi = new MedicoDomicilio();
		...
		try {
			...
			m.setFuerzaVenta(rs.getString("fvta"));
			m.setCodigo(rs.getString("munico"));
			for (int i=0;i<2;i++){
				domi.setDireccion(rs.getString("domicilio"+(i>1?"2":"")));
				domi.setCodPostal(rs.getString("cpa"+(i>1?"2":"")));
				domi.setLocalidad(rs.getString("localidad"+(i>1?"2":"")));
				domi.setRegion(rs.getString("region"+(i>1?"2":"")));
				m.addDomicilio(domi);
			}
